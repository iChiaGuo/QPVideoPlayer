...
@interface QPPlayerController () <QPTransportDelegate>

@property (strong, nonatomic) AVAsset *asset;
@property (strong, nonatomic) AVPlayerItem *playerItem;
@property (strong, nonatomic) AVPlayer *player;
@property (strong, nonatomic) QPPlayerView *playerView;

@property (weak, nonatomic) id <QPTransport> transport;
...
- (void)prepareToPlay {
    ...
    self.playerView = [[QPPlayerView alloc] initWithPlayer:self.player];    // 5
...
- (void)addPlayerItemTimeObserver {
    ...
    __weak QPPlayerController *weakSelf = self;                             // 3
...
- (void)addItemEndObserverForPlayerItem {
    ...
    __weak QPPlayerController *weakSelf = self;                             // 1
...
#pragma mark - QPTransportDelegate Methods
...
#pragma mark - QPumbnail Generation
...
- (void)generateThumbnails {
    ...
    handler = ^(CMTime requestedTime,
        ...
        if (result == AVAssetImageGeneratorSucceeded) {                     // 6
            ...
            id thumbnail =
                [QPThumbnail thumbnailWithImage:image time:actualTime];
        ...
        if (--imageCount == 0) {                                            // 7
            dispatch_async(dispatch_get_main_queue(), ^{
                NSString *name = QPThumbnailsGeneratedNotification;
